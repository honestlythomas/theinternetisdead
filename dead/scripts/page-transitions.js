...
(() => {
  ...
  const installNativeStyles = () => {
    ...
    style.textContent = `
      ...
      @keyframes dead-glitch-out {
        ...
        55% { transform: translate3d(14px,-3px,0) skewX(-4deg); clip-path: inset(24% 0 18%); }
        78% { transform: translate3d(-24px,4px,0) skewX(8deg); clip-path: inset(42% 0 38%); opacity: .6; }
        100% { transform: translate3d(42px,0,0) scaleX(1.08); clip-path: inset(49% 0); opacity: 0; }
      }
      @keyframes dead-glitch-in {
        0% { transform: translate3d(-42px,0,0) scaleX(1.08); clip-path: inset(49% 0); opacity: 0; }
        45% { transform: translate3d(14px,-3px,0) skewX(-4deg); clip-path: inset(24% 0 18%); opacity: .7; }
        100% { transform: translate3d(0,0,0); clip-path: inset(0); opacity: 1; }
      }
      ...
      @keyframes dead-crt-out {
        0% { transform: scale(1); opacity: 1; }
        72% { transform: scale(1,.008); opacity: 1; }
        100% { transform: scale(0,.008); opacity: 0; }
      }
      @keyframes dead-crt-in {
        0% { transform: scale(0,.008); opacity: 0; }
        28% { transform: scale(1,.008); opacity: 1; }
  ...
  const makeSignalOverlay = effect => {
    ...
    Object.assign(field.style, {
      position: "absolute",
      inset: "-24%",
      background: effect.startsWith("slide-")
        ? "linear-gradient(90deg, transparent 34%, rgba(114,255,25,.16) 48%, rgba(255,43,214,.3) 50%, rgba(0,225,255,.18) 52%, transparent 66%)"
        : "radial-gradient(circle at 50% 50%, rgba(0,0,0,.9) 0 5%, transparent 24%), conic-gradient(from 0deg, rgba(114,255,25,.18), transparent 18%, rgba(255,43,214,.28), transparent 52%, rgba(0,225,255,.2), transparent 82%, rgba(114,255,25,.18))",
      opacity: "0",
    ...
    Object.assign(scan.style, {
      position: "absolute",
      inset: "0",
      background: "repeating-linear-gradient(0deg, transparent 0 4px, rgba(114,255,25,.11) 4px 5px, transparent 5px 8px), linear-gradient(90deg, rgba(255,43,214,.08), transparent 40%, rgba(0,225,255,.08))",
      opacity: "0",
      willChange: "transform, opacity"
    });
  ...
  const reverseFrames = frames => frames.map(frame => {
    const copy = { ...frame };
    delete copy.offset;
    return copy;
  }).reverse();
  ...
  const rootFrames = (effect, outgoing) => {
    ...
    if (slideVectors[effect]) {
      ...
      return outgoing
        ? [{ transform: "translate3d(0,0,0)", opacity: 1 }, { transform: `translate3d(${outX},${outY},0)`, opacity: 1 }]
        : [{ transform: `translate3d(${inX},${inY},0)`, opacity: 1 }, { transform: "translate3d(0,0,0)", opacity: 1 }];
    }
    ...
    if (effect === "glitch-disintegrate") return outgoing ? [
      { transform: "translate3d(0,0,0)", clipPath: "inset(0)", opacity: 1 },
      { transform: "translate3d(-10px,2px,0) skewX(2deg)", clipPath: "inset(8% 0 12%)", opacity: .95, offset: .28 },
      { transform: "translate3d(14px,-3px,0) skewX(-4deg)", clipPath: "inset(24% 0 18%)", opacity: .8, offset: .55 },
      { transform: "translate3d(-24px,4px,0) skewX(8deg)", clipPath: "inset(42% 0 38%)", opacity: .55, offset: .78 },
      { transform: "translate3d(42px,0,0) scaleX(1.08)", clipPath: "inset(49% 0)", opacity: 0 }
    ] : [
      { transform: "translate3d(-42px,0,0) scaleX(1.08)", clipPath: "inset(49% 0)", opacity: 0 },
      { transform: "translate3d(14px,-3px,0) skewX(-4deg)", clipPath: "inset(24% 0 18%)", opacity: .72, offset: .45 },
      { transform: "translate3d(0,0,0)", clipPath: "inset(0)", opacity: 1 }
    ];

    if (effect === "void-iris") return outgoing
      ? [{ clipPath: "circle(150% at 50% 50%)", transform: "scale(1)", opacity: 1 }, { clipPath: "circle(0 at 50% 50%)", transform: "scale(.985)", opacity: .2 }]
      : [{ clipPath: "circle(0 at 50% 50%)", transform: "scale(.985)", opacity: .2 }, { clipPath: "circle(150% at 50% 50%)", transform: "scale(1)", opacity: 1 }];

    return outgoing ? [
      { transform: "scale(1,1)", opacity: 1 },
      { transform: "scale(1,.008)", opacity: 1, offset: .72 },
      { transform: "scale(0,.008)", opacity: 0 }
    ] : [
      { transform: "scale(0,.008)", opacity: 0 },
      { transform: "scale(1,.008)", opacity: 1, offset: .28 },
      { transform: "scale(1,1)", opacity: 1 }
    ];
  };
  ...
  const playEffect = async (effect, phase, options = {}) => {
    ...
    if (reducedMotion) {
      const animation = trackAnimation(root.animate(outgoing
        ? [{ opacity: 1 }, { opacity: 0 }]
        : [{ opacity: 0 }, { opacity: 1 }], { duration, easing, fill: "both" }));
    ...
    const animations = [
      trackAnimation(root.animate(rootFrames(effect, outgoing), timing)),
      trackAnimation(field.animate(signal.field, timing)),
      trackAnimation(scan.animate(signal.scan, { ...timing, easing: effect === "glitch-disintegrate" ? "steps(8,end)" : "linear" }))
    ];
  ...
  const canUseNativeNavigation = (from, to) => nativeNavigation
    && NATIVE_PATHS.has(from)
    && NATIVE_PATHS.has(to);
  ...
  const api = {
    ...
    profile: Object.freeze({
      nativeNavigation,
      reducedMotion,
      maximumAnimatedLayers: 3,
      destinationFetches: 0,
      pageClones: 0
    }),
    transitions,
    routes
  ...
  ["swirl", "glitch-disintegrate", "void-iris", "crt-collapse"].forEach(effect => {
    api.register(effect, {
      out: options => playEffect(effect, "out", options),
      in: options => playEffect(effect, "in", options)
    });
  });
  ...
  window.addEventListener("pageshow", event => {
    ...
    leaving = false;
    cleanup(transitionRoot({}));
    delete document.documentElement.dataset.deadTransition;
  });
